...
describe('ScheduleService', () => {
    ...
    describe('getScheduleById', () => {
        it('should return a schedule when it exists', async () => {
            prisma.schedule.findUnique = jest.fn().mockReturnValueOnce(mockScheduleResult);
    ...
    describe('updateSchedule', () => {
        it('should update and return the updated schedule', async () => {
            prisma.schedule.update = jest.fn().mockReturnValueOnce(mockScheduleResult);

            const schedule = await service.updateSchedule(mockScheduleId, mockCreateScheduleInput);
        ...
        it('should throw NotFoundException if schedule does not exist', async () => {
            prisma.schedule.update = jest.fn().mockRejectedValueOnce(
                new Prisma.PrismaClientKnownRequestError('error',
                    { code: PrismaErrorCode.RecordNotFound, clientVersion: 'any' }));

            await expect(service.updateSchedule(mockScheduleId, { account_id: 321 })).rejects.toThrow(
                new NotFoundException(`Schedule with ID ${mockScheduleId} not found`),
            );
        });
    });
    ...
    describe('deleteSchedule', () => {
        it('should delete and return the deleted schedule', async () => {
            prisma.schedule.delete = jest.fn().mockResolvedValueOnce(mockScheduleResult);

            const schedule = await service.deleteSchedule(mockScheduleId);
            expect(prisma.schedule.delete).toHaveBeenCalledWith({ where: { id: mockScheduleId } });
            expect(schedule).toEqual(mockScheduleResult);
        });

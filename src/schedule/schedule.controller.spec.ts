...
describe('ScheduleController', () => {
  ...
  describe('getScheduleById', () => {
    it('should return schedule by id', async () => {
      scheduleService.getScheduleById = jest
        .fn()
        .mockReturnValueOnce(mockScheduleResult);
      expect(await scheduleController.getScheduleById(mockScheduleId)).toEqual(
        mockScheduleResult,
      );
    });
  });
  ...
  describe('createSchedule', () => {
    it('should return created schedule', async () => {
      scheduleService.createSchedule = jest
        ...
        .mockReturnValueOnce(mockScheduleResult);
      expect(
        await scheduleController.createSchedule(mockCreateScheduleInput),
      ).toEqual(mockScheduleResult);
    });
  });

  describe('deleteSchedule', () => {
    it('should return delete schedule', async () => {
      scheduleService.deleteSchedule = jest
        .fn()
        .mockReturnValueOnce(mockScheduleResult);
      expect(await scheduleController.deleteSchedule(mockScheduleId)).toEqual(
        mockScheduleResult,
      );
    });
  });
